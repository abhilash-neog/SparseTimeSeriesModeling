...
from utils import MaskEmbed, MAEDataset, NativeScaler, get_1d_sincos_pos_embed, ActiveEmbed, adjust_learning_rate
eps = 1e-6
...
class MaskedAutoencoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder_pred = nn.Linear(self.decoder_embed_dim, self.num_feats, bias=True)  # decoder to patch
...
#         self.decoder_pos_embed.data.copy_(torch.from_numpy(decoder_pos_embed).float().unsqueeze(0))
    ...
    def set_masking_mode(self, masking_mode=None):
        ...
        print(f"Masking Mode has been initialized with default {self.masking_mode}")
    ...
    def forward_loss(self, data, pred, mask, nask, masked_penalize=False):
        """
        data: [N, 1, L]
        pred: [N, L]
    ...
    def freeze_encoder_model(self):
        ...
        print(f"Emcoder Blocks Frozen!")
    
    def train_model(self, Xtrain, Xval, args, masked_penalize=False):
        ...
        self.mpl = ModelPlugins(seq_len=self.rec_len, enc_embed_dim=self.embed_dim, dec_embed_dim=self.decoder_embed_dim, device=self.device)
            ...
            for it in tr:
                ...
                for iteration, (samples, masks) in enumerate(dataloader):
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        loss, masked_loss, unmasked_loss = self.forward_loss(samples, pred, mask, nask, masked_penalize)
                ...
                if self.mask_ratio<1 and masked_penalize==False:
                    metrics = {
                        ...
                        "train_mse":batch_train_loss_dict["MSE"],
                        "val_mse":batch_val_loss_dict["MSE"]
                    }
                else:
                    metrics = {
                        "mse_loss":batch_loss,
                ...
                if self.feature_wise_mse=='True':
                    batch_train_loss_dict.pop("MSE")
                    batch_val_loss_dict.pop("MSE")
        ...
        with torch.no_grad():
            ...
            if self.feature_wise_mse=='True':
                for k,v in val_eval_dict["mse_dict"].items():
                    if k=='MSE':
                        wandb.summary['val_mse'] = v
                    else:
                        wandb.summary[k] = v
                
                for k,v in train_eval_dict["mse_dict"].items():
                    if k=='MSE':
                        wandb.summary['train_mse'] = v
                    else:
                        wandb.summary[k] = v
                        
            else:
                wandb.summary['val_mse'] = val_eval_dict["mse_dict"]['MSE'].item()
                wandb.summary['train_mse'] = train_eval_dict["mse_dict"]['MSE'].item()
            # wandb.finish()
        
            return losses
    
    def evaluate_and_plot(self, Xtrain, Xval, args, it=0, num_windows=25, num_samples=8):
        val_eval_dict = self.evaluate(val_X=Xval, args=args, train_or_val="VAL")
        train_eval_dict = self.evaluate(val_X=Xtrain, args=args, train_or_val="TRAIN")
        
        batch_val_loss = val_eval_dict["mse_dict"]
        batch_train_loss = train_eval_dict["mse_dict"]
        ...
        self.utils.plot_context_window_grid(val_x_gt, val_predictions, val_masks, 
                                            val_start, it, 'Val', title_prefix=self.masking_mode)
        self.utils.plot_context_window_grid(train_x_gt, train_predictions, train_masks, 
        ...
        if self.masking_mode == "random_masking":
            ...
            self.utils.plot_merged_context_windows(val_x_gt, val_predictions, val_masks, 
                                                   val_start, it, 'Val', title_prefix=self.masking_mode)
            self.utils.plot_merged_context_windows(train_x_gt, train_predictions, train_masks, 
                                                   train_start, it, 'Train', title_prefix=self.masking_mode)
        elif self.masking_mode == "continuous_masking":
            ...
            self.utils.plot_forecast(val_x_gt, val_predictions, 
                                  val_idx, plt_idx, self.lookback_window ,it, 'Val', title_prefix=self.masking_mode)
            self.utils.plot_forecast(train_x_gt, train_predictions,
                                  train_idx, plt_idx, self.lookback_window, it, 'Train', title_prefix=self.masking_mode)
            
        return batch_train_loss, batch_val_loss
    
    def forecast_evaluate(self, Xtrain, Xval, args, lookback):
        self.set_lookbackwindow(lookback)
        self.set_masking_mode(masking_mode="continuous_masking")
        ...
        _, _ = self.evaluate_and_plot(Xtrain, Xval, args, num_windows=8)
    ...
    def predict(self, X, args):
        ...
        for it, (samples, masks) in tqdm(enumerate(dataloader)):
            ...
            with torch.cuda.amp.autocast():
                ...
                loss, _, _ = self.forward_loss(samples, pred, mask, nask)
    ...
    def evaluate(self, val_X, args, train_or_val):
        ...
        predictions = predictions*self.utils.feat_std.to(self.device) + self.utils.feat_mean.to(self.device)
        val_X = val_X*self.utils.feat_std.to(self.device) + self.utils.feat_mean.to(self.device)
        
        MSE_dict = {}
        if self.task_name == "pretrain":
            if self.feature_wise_mse=='True':
                feature_wise_mse = ((predictions-val_X)**2).mean(dim=(0, 1))
                MSE_dict = {train_or_val+"_"+self.utils.inp_cols[idx]:feature_wise_mse[idx].item() for idx in range(self.num_feats)}
            
            MSE = (((predictions-val_X)**2).mean())
            MSE_dict["MSE"] = MSE.item()
            
        elif self.task_name == "finetune":
            if self.feature_wise_mse=='True':
                sqred_err = (predictions-val_X)**2
                sum_sqred_err = (sqred_err*masks.unsqueeze(-1)).sum((0,1))
                feature_wise_mse = (sum_sqred_err/masks.unsqueeze(-1).repeat(1,1,sqred_err.shape[2]).sum((0,1)))
                MSE_dict = {train_or_val+"_"+self.utils.inp_cols[idx]:feature_wise_mse[idx].item() for idx in range(self.num_feats)}
            
            sqred_err = (predictions-val_X)**2
            MSE = ((sqred_err*masks.unsqueeze(-1)).sum() / masks.unsqueeze(-1).repeat(1,1,sqred_err.shape[2]).sum())
            MSE_dict["MSE"] = MSE.item()

        # mse1 = (((pred-gt)**2).mean())**0.5
        # mse2 = ((((pred-gt)**2).mean(dim=(-2, -1)))**0.5).mean()
        return {'avg_loss':loss, 'mse_dict':MSE_dict, 'preds':predictions, 'gt': val_X, 'mask': masks}
