...
class ClassifierHead(nn.Module):
    def __init__(self, configs, d_model, seq_len, n_vars=1, head_dropout=0):
        super().__init__()
        self.flatten = nn.Flatten(start_dim=-3)
        self.logits = nn.Linear(seq_len*d_model*n_vars, 64)
...
class Model(nn.Module):
    ...
    def __init__(self, configs, data_config):
        ...
        self.num_feats = num_feats[self.configs.target_dataset]
        ...
        if self.task_name == 'pretrain':
            ...
            self.head = ClassifierHead(seq_len=configs.seq_len, 
                                     d_model=configs.d_model, 
                                     configs=self.data_config, 
                                     head_dropout=configs.head_dropout,
                                     n_vars=self.num_feats)
...
#         exit(0)
        ...
        # decoder
        dec_out = self.head(enc_out)  # dec_out: [bs x num_classes]
