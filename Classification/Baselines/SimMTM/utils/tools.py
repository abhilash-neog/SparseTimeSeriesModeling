...
class EarlyStopping:
    def __init__(self, patience=7, args=None, verbose=False, delta=0, root_path=None):
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        # torch.save(model.state_dict(), path + '/' + 'checkpoint_'+self.root_path.split('/')[-1]+'.pth')
        # print(f"path = {path + '/' + 'checkpoint_'+self.root_path.split('/')[-1]+'.pth'}")
        torch.save(model.state_dict(), path + '/' + 'checkpoint_'+self.args.fraction+'.pth')
        print(f"path = {path + '/' + 'checkpoint_'+self.args.fraction+'.pth'}")
...
def transfer_weights(weights_path, model, exclude_head=True, device='cpu'):
    ...
    print(f"weights being loaded from {weights_path}")
