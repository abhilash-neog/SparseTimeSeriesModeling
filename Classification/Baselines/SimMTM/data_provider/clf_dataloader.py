...
class Load_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.x_data[index].permute(1,0), self.y_data[index]

    def __len__(self):
        return self.len


def data_generator(sourcedata_path, targetdata_path, configs, trial, imputation, training_mode, subset = True, fraction=''):
    
    if fraction != '':
        if imputation=="Spline":
            train_pt = "train_"+str(fraction)+"_spline_imputed" + "__v" + str(trial) + ".pt"
            val_pt = "val_"+str(fraction)+"_spline_imputed" + "__v" + str(trial) + ".pt"
            test_pt = "test_"+str(fraction)+"_spline_imputed" + "__v" + str(trial) + ".pt"
        else:   
            train_pt = "train_"+str(fraction)+"_SAITS_imputed" + "_v" + str(trial) + ".pt"
            val_pt = "val_"+str(fraction)+"_SAITS_imputed" + "_v" + str(trial) + ".pt"
            test_pt = "test_"+str(fraction)+"_SAITS_imputed" + "_v" + str(trial) + ".pt"
    else:
        train_pt = "train.pt"
        val_pt = "val.pt"
        test_pt = "test.pt"
    ...
    if training_mode=='pre_train':
        train_dataset = torch.load(os.path.join(sourcedata_path, train_pt))
        finetune_dataset = torch.load(os.path.join(sourcedata_path, train_pt))
        test_dataset = torch.load(os.path.join(sourcedata_path, test_pt))
    else:
        train_dataset = torch.load(os.path.join(targetdata_path, train_pt))
        finetune_dataset = torch.load(os.path.join(targetdata_path, val_pt))
        test_dataset = torch.load(os.path.join(targetdata_path, test_pt))
