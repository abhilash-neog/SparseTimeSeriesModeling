...
out_filename = f"v{{}}_{masking_type}_{dataset.lower()}_imputed_LOCF.csv"
...
for trial in tqdm(range(5)):
    ...
    train_df_interpolated = train_df.fillna(method='ffill').fillna(method='bfill')
    val_df_interpolated = val_df.fillna(method='ffill').fillna(method='bfill')
    test_df_interpolated = test_df.fillna(method='ffill').fillna(method='bfill')
