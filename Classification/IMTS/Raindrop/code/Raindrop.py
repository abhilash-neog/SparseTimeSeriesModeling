...
arch = 'raindrop'
# Save directory override per user request
model_path = '/raid/abhilash/imts_baselines/raindrop/models'
...
for missing_ratio in missing_ratios:
    ...
    # Arrays to store metrics across splits
    avg_epoch_time_arr = np.zeros((n_splits, n_runs))
    inference_throughput_arr = np.zeros((n_splits, n_runs))
    peak_memory_gb_arr = np.zeros((n_splits, n_runs))
    for k in range(n_splits):
        ...
        for m in range(n_runs):
            ...
            for epoch in range(num_epochs):
                ...
                if epoch >= 1:  # Start tracking from epoch 2 (index 1, since 0-indexed)
                    ...
                    if len(epoch_times) >= 6:  # Track for 6 epochs
                        ...
                        print(f'Epoch {epoch}: Time = {epoch_time:.3f}s | Avg (last 6): {avg_epoch_time:.3f}s')
                
                # Check peak GPU memory after 2-3 epochs
                if epoch == 2 and torch.cuda.is_available():
                    peak_memory_bytes = torch.cuda.max_memory_allocated(device)
                    peak_memory_gb = peak_memory_bytes / (1024 ** 3)
                    peak_memory_gb_arr[k, m] = peak_memory_gb  # Store for this split/run
            ...
            if len(epoch_times) >= 6:
                ...
                avg_epoch_time_arr[k, m] = avg_training_time_per_epoch  # Store for this split/run
                print('=' * 60)
    ...
    # Aggregate metrics across splits (using best run per split)
    avg_epoch_time_vec = [avg_epoch_time_arr[k, idx_max[k]] for k in range(n_splits)]
    inference_throughput_vec = [inference_throughput_arr[k, idx_max[k]] for k in range(n_splits)]
    peak_memory_gb_vec = [peak_memory_gb_arr[k, idx_max[k]] for k in range(n_splits)]
    ...
    print('------------------------------------------')
    print('PERFORMANCE METRICS (across %d splits):' % n_splits)
    mean_epoch_time, std_epoch_time = np.mean(avg_epoch_time_vec), np.std(avg_epoch_time_vec)
    mean_throughput, std_throughput = np.mean(inference_throughput_vec), np.std(inference_throughput_vec)
    mean_memory, std_memory = np.mean(peak_memory_gb_vec), np.std(peak_memory_gb_vec)
