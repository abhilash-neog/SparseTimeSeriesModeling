...
from utils.utils import FeatEmbed
...
class SimpleTransformer(nn.Module):
    """
    Simple Transformer for Classification with Missing Data Handling
    Keeps the key contributions: mask_embed and cross_attention for missing data
    """
    ...
    def __init__(self,
        ...
        super().__init__()
        print(f"SimpleTransformer - Number of features = {num_feats}")
        ...
        # Key components for missing data handling (your contributions)
        ...
        # Feature embedding for missing data handling
    ...
    def cross_attention(self, x, m):
        """Cross-attention mechanism to handle missing data patterns"""
    ...
    def forward_encoder(self, x, m):
        """Forward pass through encoder with missing data handling"""
        
        # Automatically extract the last seq_len elements if input is longer
        if x.shape[1] > self.seq_len:
            x = x[:, -self.seq_len:, :]
            m = m[:, -self.seq_len:, :]
        
        # Embed features (handles missing data)
        x = self.mask_embed(x)
        
        # Add positional embeddings if available
        if hasattr(self, 'mpl') and self.mpl is not None:
            actual_seq_len = x.shape[1]
            pos_embed_seq_len = self.mpl.pos_embed.shape[1] - 1  # -1 for cls token
            
            if actual_seq_len <= pos_embed_seq_len:
                x = x + self.mpl.pos_embed[:, 1:actual_seq_len+1, :, :]
        
        # Cross-attention for missing data handling (your contribution)
    ...
    def forward(self, data, miss_idx, mpl=None):
        """Forward pass for classification"""
        self.mpl = mpl
        
        # Encode with missing data handling
        latent = self.forward_encoder(data, miss_idx)
        
        # Remove cls token for classification head
        latent = latent[:, 1:, :]
...
#             mask[m[:, 0, :] < eps] = 0
    ...
    def cross_attention(self, x, m):
        ...
        # print(f"var_query device: {var_query.device}, x device: {x.device}, m_ device: {m_.device}")
        attn_out, _ = self.mhca(var_query, x, x, key_padding_mask=m_)
