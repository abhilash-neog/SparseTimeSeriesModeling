...
class FlattenHead(nn.Module):
    ...
    def __init__(self, args, num_feats):
        ...
        self.decoder_inp = nn.Linear(self.encoder_embed_dim, self.pred_len, bias=True)
        # self.decoder_pred = nn.Linear(self.encoder_embed_dim, self.num_feats, bias=True)
        self.dropout = nn.Dropout(args.fc_dropout)

    def forward(self, x, means, std):  # [bs x num_feat x enc_embed_dim]
        
        # x = x.permute(0, 2, 1) # [bs x enc_embed_dim x seq_len]
        x = self.decoder_inp(x) # [bs x num_feat x pred_len]
        x = x.permute(0, 2, 1) # [bs x pred_len x num_feat]
...
class MaskedAutoencoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.mhca = nn.MultiheadAttention(embed_dim=self.embed_dim, kdim=self.kdim, vdim=self.vdim, num_heads=self.num_heads, batch_first=True)

        self.mask_embed = FeatEmbed(input_dim=self.seq_len,
                                    embedding_dim=self.kdim,
...
#             mask[m[:, 0, :] < eps] = 0
    ...
    def cross_attention(self, x, m):
        ...
        attn_out = attn_out.reshape(batch_size, num_feat, self.embed_dim)
    ...
    def forward_encoder(self, x, m):
        ...
        cls_tokens = self.cls_token.expand(x.shape[0], -1, -1, -1)
    ...
    def forward_decoder(self, x, ids_restore, means, std):
        ...
        # print(f"x shape after decoder blocks = {x.shape}")
    ...
    def forward_loss(self, data, pred, miss_idx, mask=None, nask=None, masked_penalize=False):
        ...
        if mask is not None:
            mask = mask.unsqueeze(-1) * torch.ones(1, pred.shape[1], device=mask.device)
