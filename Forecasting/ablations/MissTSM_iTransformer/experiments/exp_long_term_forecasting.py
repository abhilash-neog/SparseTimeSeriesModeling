...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, mask_x, mask_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                mask_x = mask_x.float().to(self.device)
                mask_y = mask_y.float().to(self.device)
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs = self.model(batch_x, mask_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                        else:
                            outputs = self.model(batch_x, mask_x, batch_x_mark, dec_inp, batch_y_mark)
                else:
                    if self.args.output_attention:
                        outputs = self.model(batch_x, mask_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                    else:
                ...
                y_mask = mask_y.detach().cpu()
                
                # loss = criterion(pred, true)
                loss = self.forward_loss(pred, true, y_mask)
                total_loss.append(loss)
        total_loss = np.average(total_loss)
        self.model.train()
        return total_loss

    def forward_loss(self, outputs, batch_y, mask_y):
        target = batch_y
        
        loss = (outputs - target) ** 2
        
        loss = (loss * mask_y).sum() / mask_y.sum()
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, mask_x, mask_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                iter_count += 1
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        loss = self.forward_loss(outputs, batch_y, mask_y)
    ...
    def test_masked(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, mask_x, mask_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                mask_x = mask_x.float().to(self.device)
                mask_y = mask_y.float().to(self.device)
            ...
            for i, (batch_x, batch_y, mask_x, mask_y, batch_x_mark, batch_y_mark) in enumerate(test_loader_gt):
                # batch_x = batch_x.float().to(self.device)
                # batch_y = batch_y.float().to(self.device)
