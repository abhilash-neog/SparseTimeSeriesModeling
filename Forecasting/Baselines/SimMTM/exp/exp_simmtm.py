...
class Exp_SimMTM(Exp_Basic):
    ...
    def pretrain(self):
        ...
        path = os.path.join(self.args.pretrain_checkpoints, self.args.data + "_" + str(self.args.trial))
        print(f"pretrained model save to {path}")
...
#             stdev_ls = torch.cat(stdev_ls, dim=0).cpu().numpy()
#             means_ls = torch.cat(means_ls, dim=0).cpu().numpy()
...
#             for i in range(3):  # Loop over each feature
#                 plt.plot(stdev_ls[:100, i], label=f'Feature {i+1}')  # Plot the curve for the i-th feature

#             plt.xlabel('Samples')
#             plt.ylabel('Std deviation per sample')
#             plt.title('Plot of Each Feature - STD DEV')
#             plt.legend()

#             # Plot each feature as a curve for the second array
#             plt.subplot(1, 2, 2)  # Select the second plot
#             for i in range(3):  # Loop over each feature
#                 plt.plot(means_ls[:100, i], label=f'Feature {i+1}')  # Plot the curve for the i-th feature

#             plt.xlabel('Samples')
#             plt.ylabel('Means per sample')
#             plt.title('Plot of Each Feature - MEANS')
#             plt.legend()
